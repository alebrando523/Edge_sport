...
// Controlla se due date sono compatibili entro una tolleranza in ore
function datesCompatible(dateA, dateB, toleranceHours = 24) {
  if (!dateA || !dateB) return true; // se manca una data non scartiamo
...
function isMatch(oddsEv, polyEv) {
  // Prima controlla la compatibilità di data/ora — tolleranza 24h
...
function calcArbitrage(oddsEv, polyEv, budget = 100) {
  ...
  for (const [oddsTeamA, oddsQuotaA] of oddsOutcomes) {
    for (const [polyTeamB, polyPriceB] of polyOutcomes) {
      ...
      // Scarta quote bookmaker irrealistiche (> 10.0 = prob < 10%, non è win/lose normale)
      if (oddsQuotaA > 10.0) continue;
      ...
      // Verifica coerenza cross-match:
      // oddsTeamA (BK) deve corrispondere a polyTeamA (il lato opposto su PM)
      // oddsTeamB (BK) deve corrispondere a polyTeamB (il lato che scommettiamo su PM)
      const crossMatchA = overlap(oddsTeamA, polyTeamA);
      const crossMatchB = overlap(oddsTeamB, polyTeamB);
      // Almeno uno dei due lati deve avere un match chiaro
      if (crossMatchA < 0.4 && crossMatchB < 0.4) continue;
      // I due lati PM non devono entrambi riferirsi alla stessa squadra BK
      const pmBmatchesOddsA = overlap(oddsTeamA, polyTeamB);
      const pmBmatchesOddsB = overlap(oddsTeamB, polyTeamB);
      if (pmBmatchesOddsA > 0.5 && pmBmatchesOddsB < 0.3) continue; // stesso lato, non opposto
